...
- An extraction map first has to solve an in-match economy problem: why resources pull players in, what risks each route creates, and how players can extract with the value they found.
...
Extraction level design is not about making a map bigger or spreading resource points, AI, and extraction points evenly. It has to answer three concrete questions: why players are willing to take risks, who can read that risk, and whether the player can leave with the value they found.
...
*Note: The PvE function matrix diagram generated by image2 puts four AI functions into the same player route: economy source, information alarm, status drain, and regional door lock. The exact refresh rate is not expressed in the figure, only the level function. *

PvE has at least four functions in Search and Fight:

| PvE features | Placement | effect |
|---|---|---|
| Economy source | Low risk outer circle | Start income and basic rhythm for low-level players |
